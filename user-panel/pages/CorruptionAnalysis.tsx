// rezking07/eradicator-ai/ERADICATOR-AI-c97fdc3b5374f6cffe6641c8f996ff599a2f96b6/user-panel/pages/CorruptionAnalysis.tsx
...
import { AnalysisResult, ChatMessage, InsightItem } from '../types';
...
const CorruptionAnalysis: React.FC = () => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage: ChatMessage = { id: Date.now() + 1, sender: 'ai', text: t('aiError') };
    ...
    const translateRisk = (risk: AnalysisResult['riskLevel']) => {
        const map = { 'High': t('riskHigh'), 'Medium': t('riskMedium'), 'Low': t('riskLow') };
        return map[risk];
    }
            ...
            {analysisResult && (
                <>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="mt-2 space-y-3">
                                    {analysisResult.insights.map((insight: InsightItem, index: number) => (
                                        <div key={index} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900/50">
                                            <p className="font-semibold text-gray-800 dark:text-gray-200">
                                                <span className="font-bold text-red-500">[{insight.type.replace('_', ' ')}]</span> {insight.itemName}
                                            </p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{insight.details}</p>
                                            <p className="text-sm text-gray-700 dark:text-gray-300 mt-2"><b>Rekomendasi:</b> {insight.recommendation}</p>
                                            {insight.potentialLoss > 0 && (
                                                <p className="text-sm font-bold text-red-600 dark:text-red-400 mt-2">
                                                    Estimasi Potensi Kerugian: Rp{insight.potentialLoss.toLocaleString('id-ID')}
                                                </p>
                                            )}
                                        </div>
                                    ))}
                    ...
                    <Card>
                        ...
                        <div className="h-96 flex flex-col border rounded-lg dark:border-gray-700">
                             <div className="flex-grow overflow-y-auto p-4 space-y-4">
                                {discussionMessages.map(message => (
                                    <div key={message.id} className={`flex items-start gap-3 ${message.sender === 'user' ? 'justify-end' : ''}`}>
                                        {message.sender === 'ai' && ( <div className="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center text-white flex-shrink-0"><BotIcon className="w-5 h-5" /></div> )}
                                        <div className={`max-w-md p-3 rounded-lg ${message.sender === 'user' ? 'bg-primary-600 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
                                            <div className="prose prose-sm dark:prose-invert max-w-none"><MarkdownRenderer content={message.text} /></div>
                                        </div>
                                         {message.sender === 'user' && ( <div className="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-gray-600 dark:text-gray-300 flex-shrink-0"><UserIcon className="w-5 h-5" /></div> )}
                                    </div>
                                ))}
                                 {isAiResponding && (
                                     <div className="flex items-start gap-3">
                                        <div className="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center text-white flex-shrink-0"><BotIcon className="w-5 h-5" /></div>
                            ...
                            <div className="p-2 border-t border-gray-200 dark:border-gray-700">
                                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                                    ...
                                    />
                                    <Button type="submit" disabled={isAiResponding || !discussionInput.trim()}><SendIcon className="w-5 h-5"/></Button>
                                </form>
                            </div>
                        </div>
                    </Card>
